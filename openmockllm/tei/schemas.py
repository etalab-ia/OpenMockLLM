...
#   timestamp: 2026-01-13T07:11:13+00:00
...
class EmbedAllResponse(RootModel[list[list[list[float]]]]):
    root: list[list[list[float]]] = Field(..., examples=[[[[0.0, 1.0, 2.0]]]])


class EmbedResponse(RootModel[list[list[float]]]):
    root: list[list[float]] = Field(..., examples=[[[0.0, 1.0, 2.0]]])
...
class Rank(BaseModel):
    ...
    text: str | None = Field('null', examples=['Deep Learning is ...'])
...
class SimilarityResponse(RootModel[list[float]]):
    root: list[float] = Field(..., examples=[[0.0, 1.0, 0.5]])
...
class TokenizeRequest(BaseModel):
    add_special_tokens: bool | None = Field('true', examples=['true'])
...
class TokenizeResponse(RootModel[list[list[SimpleToken]]]):
    root: list[list[SimpleToken]] = Field(
        ...,
        examples=[
            [[{'id': 0, 'special': False, 'start': 0, 'stop': 2, 'text': 'test'}]]
        ],
    )
...
class DecodeRequest(BaseModel):
    ids: InputIds
    skip_special_tokens: bool | None = Field('true', examples=['true'])
...
class Info(BaseModel):
    auto_truncate: bool
    docker_label: str | None = Field(None, examples=['null'])
    max_batch_requests: conint(ge=0) | None = Field('null', examples=['null'])
    ...
    model_sha: str | None = Field(
        None, examples=['fca14538aa9956a46526bd1d0d11d69e19b5a101']
    )
    model_type: ModelType
    sha: str | None = Field(None, examples=['null'])
...
class OpenAICompatRequest(BaseModel):
    dimensions: conint(ge=0) | None = Field('null', examples=['null'])
    encoding_format: EncodingFormat | None = 'float'
    input: Input
    model: str | None = Field(None, examples=['null'])
    user: str | None = Field(None, examples=['null'])
...
class PredictRequest(BaseModel):
    inputs: PredictInput
    raw_scores: bool | None = Field('false', examples=['false'])
    truncate: bool | None = Field('false', examples=['false'])
    truncation_direction: TruncationDirection | None = 'right'
...
class RerankRequest(BaseModel):
    ...
    raw_scores: bool | None = Field('false', examples=['false'])
    return_text: bool | None = Field('false', examples=['false'])
...
class SimilarityRequest(BaseModel):
    ...
    parameters: SimilarityParameters | None = Field(
        default_factory=lambda: SimilarityParameters.model_validate('null')
    )
...
class EmbedRequest(BaseModel):
    ...
    inputs: Input
    normalize: bool | None = Field('true', examples=['true'])
    prompt_name: str | None = Field(
        'null',
        description='The name of the prompt that should be used by for encoding. If not set, no prompt\nwill be applied.\n\nMust be a key in the `sentence-transformers` configuration `prompts` dictionary.\n\nFor example if ``prompt_name`` is "query" and the ``prompts`` is {"query": "query: ", ...},\nthen the sentence "What is the capital of France?" will be encoded as\n"query: What is the capital of France?" because the prompt text will be prepended before\nany text to encode.',
        examples=['null'],
